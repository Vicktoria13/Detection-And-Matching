...
def find_harris_corners(input_img, k, window_size, threshold):
    ...
    for y in range(offset, y_range):
        for x in range(offset, x_range):
            ...
            #ponderer par une gaussienne
            windowIxx = cv2.GaussianBlur(windowIxx,(9,9),3)
            windowIxy = cv2.GaussianBlur(windowIxy,(9,9),3)
            windowIyy = cv2.GaussianBlur(windowIyy,(9,9),3)
